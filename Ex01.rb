...
def get_all_category(array) #Create method that takes an array as argument
    all_category=[] #Create array to hold category
    array.each do |n| #Run through array
        all_category<<n[:violation_category] #Add the category to an array
    end
    return all_category #Return category array
end

def count(array) #Create method that takes an array as an argument
    array.tally #Count how many times each violation is in the array
end

puts count(get_all_category(violations)) #Call the count method using the array with all the violations and print it



# Problem: Find the total money owed for each violation category

def get_money(array,type) #Create method that takes array and type of violation as arguments
    money=[] #Create array to hold money
        new_array=array.select do |n| n[:violation_category]==type #Run through array and add any hash with the same violation category to a new array
    end
    new_array.each do |n| #Run through new array
        money<<n[:money_owed] #Add money owed to new array
    end
    money.sum #Get sum and return
end
puts "Total for Garbage and Refuse is #{get_money(violations,"Garbage and Refuse")}" #Print total money owed for violation
puts "Total for Unsanitary Conditions is #{get_money(violations,"Unsanitary Conditions")}" #Print total money owed for violation
puts "Total for Animals and Pests is #{get_money(violations,"Animals and Pests")}" #Print total money owed for violation
puts "Total for Building Conditions is #{get_money(violations,"Building Conditions")}" #Print total money owed for violation
puts "Total for Vegetation is #{get_money(violations,"Vegetation")}" #Print total money owed for violation
puts "Total for Chemical Hazards is #{get_money(violations,"Chemical Hazards")}" #Print total money owed for violation
puts "Total for Biohazards is #{get_money(violations,"Biohazards")}" #Print total money owed for violation
